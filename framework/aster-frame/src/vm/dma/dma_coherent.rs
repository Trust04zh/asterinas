...
use crate::{
    ...
    vm::{
        ...
        page_table::{cache_policy_op, MapCachePolicy},
...
impl DmaCoherent {
    ...
    pub fn map(vm_segment: VmSegment, is_cache_coherent: bool) -> Result<Self, DmaError> {
        ...
        if !is_cache_coherent {
            ...
            unsafe {
                page_table
                    .protect_unchecked(&va_range, cache_policy_op(MapCachePolicy::Uncachable))
                    .unwrap();
            }
        }
...
impl Drop for DmaCoherentInner {
    fn drop(&mut self) {
        ...
        if !self.is_cache_coherent {
            ...
            unsafe {
                page_table
                    .protect_unchecked(&va_range, cache_policy_op(MapCachePolicy::WriteBack))
                    .unwrap();
            }
        }
