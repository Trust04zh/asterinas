...
use crate::{
    ...
    sync::SpinLock,
    vm::{paddr_to_vaddr, Paddr, Vaddr, VmAllocOptions, VmFrame, VmFrameVec, PAGE_SIZE},
};
    ...
    [(); C::NR_LEVELS]:,
{
    ...
    /// The number of children that are not None.
    /// This is to track if we can free the child array.
    ...
    pub(crate) fn fork(&self) -> PageTable<UserMode, E, C> {
        ...
        PageTable::<UserMode, E, C> {
            root_frame: Arc::new(SpinLock::new(PageTableFrame::<E, C> {
                ...
                child_count: root_frame.child_count,
            })),
    ...
    [(); C::NR_LEVELS]:,
{
    ...
    pub(crate) fn empty() -> Self {
        use crate::vm::HasPaddr;
        let frame = PageTableFrame::<E, C>::new();
        log::debug!("Root Frame: {:#x?} ({:#x})", frame, frame.inner.paddr());
        PageTable {
            root_frame: Arc::new(SpinLock::new(frame)),
            _phantom: PhantomData,
        }
    }
    ...
    pub(crate) unsafe fn activate_unchecked(&self) {
        activate_page_table(self.root_paddr(), MapCachePolicy::Uncachable);
    }
