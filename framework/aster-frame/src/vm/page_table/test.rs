...
#[ktest]
fn test_range_check() {
    let mut pt = PageTable::<UserMode>::new();
...
fn test_base_protect_query() {
    let mut pt = PageTable::<UserMode, PageTableEntry, BasePageTableConsts>::new();
    let from_ppn = 1..1000;
    let from = PAGE_SIZE * from_ppn.start..PAGE_SIZE * from_ppn.end;
    ...
    for (Qr { va, info }, i) in pt.query(&from).unwrap().zip(from_ppn) {
        ...
        assert_eq!(info.prop.cache, MapCachePolicy::WriteBack);
...
fn test_large_protect_query() {
    let mut pt = PageTable::<UserMode, PageTableEntry, VeryHugePageTableConsts>::new();
    let gmult = 512 * 512;
