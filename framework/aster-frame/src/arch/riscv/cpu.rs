...
/// Returns the PLIC context of this hart.
pub fn this_plic_context() -> usize {
    // A hart context is a given privilege mode on a given hart (PLIC Spec 1.0.0).
    // Generally, there are two contexts per hart: M-mode and S-mode, and
    // the context ID is even-numbered for M-mode and odd-numbered for S-mode.
    // Ref: https://wiki.osdev.org/PLIC
    this_cpu() as usize * 2 + 1
}
...
impl UserContextApiInternal for UserContext {
    fn execute(&mut self) -> crate::user::UserEvent {
        ...
        let ret = loop {
            ...
            match scause.cause() {
                Trap::Interrupt(SupervisorExternal) => call_irq_callback_functions(&self.as_trap_frame()),
