...
impl TomlManifest {
    pub fn load() -> Self {
        ...
        fn finalize(current_manifest: Option<TomlManifest>) -> TomlManifest {
            ...
            for (_key, scheme) in current_manifest.map.iter_mut() {
    ...
    pub fn get_scheme(&self, scheme: Option<impl Display>) -> &Scheme {
        if let Some(scheme) = scheme {
            let selected_scheme = self.map.get(&scheme.to_string());
            if selected_scheme.is_none() {
                error_msg!("Scheme `{}` not found in `OSDK.toml`", scheme);
