...
pub fn handle_user_signal(
    ...
    // GeneralRegs don't have pc register, so we use zero register to store pc.
    ucontext.uc_mcontext.inner.gp_regs.zero = context.instruction_pointer();
    ...
    *sig_context = Some(ucontext_addr as Vaddr);
    ...
    if flags.contains(SigActionFlags::SA_RESTORER) {
        ...
        context.set_ra(restorer_addr as _);
    ...
    context.set_a0(sig_num.as_u8() as usize); // signal number
    if flags.contains(SigActionFlags::SA_SIGINFO) {
        context.set_a1(siginfo_addr as usize); // siginfo_t* siginfo
        context.set_a2(ucontext_addr as usize); // void* ctx
    } else {
        context.set_a1(0);
        context.set_a2(0);
    }
